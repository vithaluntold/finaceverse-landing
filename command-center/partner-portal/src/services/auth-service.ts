// ============================================
// PARTNER PORTAL - Authentication Service
// Login, session management, and partner validation
// ============================================
...
const logger = pino({ name: 'partner-portal-auth-service' });
...
export interface LoginResult {
  ...
  refreshToken?: string;
  expiresIn?: number;
  user?: Omit<PortalUser, 'passwordHash' | 'totpSecret'>;
...
const AUTH_CONFIG = {
  jwtSecret: process.env.PARTNER_PORTAL_JWT_SECRET || process.env.JWT_SECRET || 'partner-portal-secret-change-in-production',
  jwtIssuer: process.env.JWT_ISSUER || 'finaceverse-partner-portal',
...
const sessions: Map<string, PortalSession> = new Map();
...
async function initializeDefaultUsers(): Promise<void> {
  const superadminPassword = process.env.PARTNER_PORTAL_ADMIN_PASSWORD || 'PartnerAdmin@2026!';
  const passwordHash = await bcrypt.hash(superadminPassword, AUTH_CONFIG.bcryptRounds);

  const superadmin: PortalUser = {
    id: 'partner-portal-superadmin-001',
    username: 'partner_admin',
    email: 'partners@finaceverse.com',
    passwordHash,
    role: 'superadmin',
    tenantId: 'platform',
    permissions: ['*'],
  ...
  logger.info('Default partner portal superadmin user initialized');
}
...
export async function createUser(userData: Omit<PortalUser, 'id' | 'passwordHash' | 'failedAttempts' | 'createdAt' | 'updatedAt'> & { password: string }): Promise<PortalUser> {
  ...
  logger.info({ userId: user.id, username: user.username, partnerId: user.partnerId }, 'Portal user created');
  
  return user;
}

export async function createPartnerUser(partnerData: {
  partnerId: string;
  username: string;
  email: string;
  password: string;
  ...
  return createUser({
    ...
    permissions: ['partner:read', 'partner:referrals', 'partner:commissions'],
    isActive: true,
    totpEnabled: false,
  });
}

export function getUser(username: string): PortalUser | undefined {
...
export function getUserByPartnerId(partnerId: string): PortalUser | undefined {
  for (const user of users.values()) {
...
export function listUsers(): Omit<PortalUser, 'passwordHash' | 'totpSecret'>[] {
...
export async function login(credentials: LoginCredentials, ip: string, userAgent: string): Promise<LoginResult> {
  ...
  logger.info({ userId: user.id, username, sessionId, partnerId: user.partnerId }, 'User logged in successfully');
...
function generateAccessToken(user: PortalUser, sessionId: string): string {
  return jwt.sign(
    {
      userId: user.id,
...
export function getSession(sessionId: string): PortalSession | undefined {
  return sessions.get(sessionId);
}

export function getUserSessions(userId: string): PortalSession[] {
